...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }) {
  ...
  useSphericalJoint(j3, card, [[0, 0, 0], [0, SPH_Y, 0]]);
  ...
  return (
    <>
      <group position={[0, ANCHOR_Y, 0]}>
        ...
        <RigidBody position={[0, -ROPE_LEN * 3, 0]} ref={j3} {...segmentProps}>
          <BallCollider args={[0.1]} />
        </RigidBody>
        <RigidBody
          position={[0, -(ROPE_LEN * 3 + SPH_Y), 0]}
          ref={card}
          {...segmentProps}
...
useGLTF.preload('/assets/lanyard/scene.glb');